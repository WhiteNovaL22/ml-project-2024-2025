...
# 收益矩阵（Player 1 用 A，Player 2 用 B）
A = np.array([[1.0, 0.0],
              [2/3, 2/3]])
...
for run in range(runs):
    ...
    for _ in range(episodes):
        # ε-greedy 策略选择动作
        a1 = random.randint(0, 1) if random.random() < epsilon else np.argmax(Q1.sum(axis=1))
        a2 = random.randint(0, 1) if random.random() < epsilon else np.argmax(Q2.sum(axis=0))
    ...
    # 最终策略选择（greedy）
    final_a1 = np.argmax(Q1.sum(axis=1))
    final_a2 = np.argmax(Q2.sum(axis=0))
