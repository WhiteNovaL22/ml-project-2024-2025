...
# encoding: utf-8
"""
Training multiple agents using Ray RLLib.
Method: IPPO: shared policy.
"""
...
class SharedRewardWrapper(BaseWrapper):
    """
    Wrapper to make all cooperating agents share the same total reward.
    """
    ...
    def step(self, action):
        ...
        # 收集当前所有弓箭手的 reward
        archer_ids = [agent for agent in self.agents if "archer" in agent]
        total_reward = sum(self.rewards.get(agent, 0.0) for agent in archer_ids)

        # 平均 reward（你也可以用总和）
        shared_reward = total_reward / len(archer_ids) if archer_ids else 0.0

        # 为所有弓箭手设置相同的 reward
        for agent in archer_ids:
            self.rewards[agent] = shared_reward

        # 注意：self.rewards 是一步步更新的，不影响历史数据。
...
class CustomPredictFunction(Callable):
    ...
    def __init__(self, env):
        ...
        best_checkpoint = (Path("results_multi") / "learner_group" / "learner" / "rl_module").resolve()
...
if __name__ == "__main__":
    ...
    training(env, checkpoint_path, max_iterations = 750)
